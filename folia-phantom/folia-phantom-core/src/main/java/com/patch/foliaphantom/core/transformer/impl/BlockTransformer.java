...
public class BlockTransformer implements ClassTransformer {
    ...
    private static class BlockClassVisitor extends ClassVisitor {
        ...
        public void visit(int version, int access, String name, String sig, String superName, String[] interfaces) {
            this.className = name;
            if (superName != null && (superName.equals("org/bukkit/plugin/java/JavaPlugin") || superName.equals("org/bukkit/plugin/PluginBase"))) {
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String sig, String[] ex) {
            ...
            if (pluginFieldName != null || isPluginClass) {
                return new BlockMethodVisitor(mv, access, name, desc, patcherPath, className, pluginFieldName, pluginFieldDesc, isPluginClass);
            }
            return mv;
        }
    }
    ...
    private static class BlockMethodVisitor extends AdviceAdapter {
        ...
        public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean isInterface) {
            ...
            if ("org/bukkit/block/Block".equals(owner) && "setType".equals(name) && "(Lorg/bukkit/Material;)V".equals(desc)) {
                ...
                storeLocal(materialLocal); // material
                int blockLocal = newLocal(Type.getType("Lorg/bukkit/block/Block;"));
                storeLocal(blockLocal); // block
            ...
            // Handle Block.setType(Material, boolean)
            if ("org/bukkit/block/Block".equals(owner) && "setType".equals(name) && "(Lorg/bukkit/Material;Z)V".equals(desc)) {
                // Stack before: [block, material, boolean]
                int applyPhysicsLocal = newLocal(Type.BOOLEAN_TYPE);
                storeLocal(applyPhysicsLocal); // boolean
                int materialLocal = newLocal(Type.getType("Lorg/bukkit/Material;"));
                storeLocal(materialLocal); // material
                int blockLocal = newLocal(Type.getType("Lorg/bukkit/block/Block;"));
                storeLocal(blockLocal); // block
                ...
                super.visitMethodInsn(INVOKESTATIC, patcherOwner, "safeSetBlockTypeWithPhysics", newDesc, false);
                return;
            }

            // Handle Block.setBlockData(BlockData)
            if ("org/bukkit/block/Block".equals(owner) && "setBlockData".equals(name) && "(Lorg/bukkit/block/data/BlockData;)V".equals(desc)) {
                // Stack before: [block, blockData]
                int blockDataLocal = newLocal(Type.getType("Lorg/bukkit/block/data/BlockData;"));
                storeLocal(blockDataLocal); // blockData
                int blockLocal = newLocal(Type.getType("Lorg/bukkit/block/Block;"));
                storeLocal(blockLocal); // block
            ...
            if ("org/bukkit/block/Block".equals(owner) && "setBlockData".equals(name) && "(Lorg/bukkit/block/data/BlockData;Z)V".equals(desc)) {
                ...
                super.visitMethodInsn(INVOKESTATIC, patcherOwner, "safeSetBlockDataWithPhysics", newDesc, false);
                return;
            }
