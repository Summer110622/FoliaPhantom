...
public final class FoliaPatcher {
  ...
  public static boolean _ap(Plugin p, Entity v, Entity ps) {
    if (Bukkit.isPrimaryThread()) return v.addPassenger(ps);
    if (FIRE_AND_FORGET) { v.getScheduler().run(p, t -> v.addPassenger(ps), null); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    v.getScheduler().run(p, t -> { try { f.complete(v.addPassenger(ps)); } catch (Exception e) { f.completeExceptionally(e); } }, null);
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
  public static boolean _rp(Plugin p, Entity v, Entity ps) {
    if (Bukkit.isPrimaryThread()) return v.removePassenger(ps);
    if (FIRE_AND_FORGET) { v.getScheduler().run(p, t -> v.removePassenger(ps), null); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    v.getScheduler().run(p, t -> { try { f.complete(v.removePassenger(ps)); } catch (Exception e) { f.completeExceptionally(e); } }, null);
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
  public static boolean _ej(Plugin p, Entity v) {
    if (Bukkit.isPrimaryThread()) return v.eject();
    if (FIRE_AND_FORGET) { v.getScheduler().run(p, t -> v.eject(), null); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    v.getScheduler().run(p, t -> { try { f.complete(v.eject()); } catch (Exception e) { f.completeExceptionally(e); } }, null);
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
  public static java.util.List<Entity> _ne(Plugin p, Entity en, double x, double y, double z) {
    if (Bukkit.isPrimaryThread()) return en.getNearbyEntities(x, y, z);
    if (FIRE_AND_FORGET) return java.util.Collections.emptyList();
    CompletableFuture<java.util.List<Entity>> f = new CompletableFuture<>();
    en.getScheduler().run(p, t -> { try { f.complete(en.getNearbyEntities(x, y, z)); } catch (Exception e) { f.completeExceptionally(e); } }, null);
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return java.util.Collections.emptyList(); }
  }
  public static boolean _ae(Plugin p, LivingEntity en, PotionEffect ef) {
    if (Bukkit.isPrimaryThread()) return en.addPotionEffect(ef);
    if (FIRE_AND_FORGET) { en.getScheduler().run(p, t -> en.addPotionEffect(ef), null); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    en.getScheduler().run(p, t -> { try { f.complete(en.addPotionEffect(ef)); } catch (Exception e) { f.completeExceptionally(e); } }, null);
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
  public static void _re(Plugin p, LivingEntity en, PotionEffectType t) {
    if (Bukkit.isPrimaryThread()) en.removePotionEffect(t);
    else en.getScheduler().run(p, task -> en.removePotionEffect(t), null);
  }
  public static Entity[] _ce(Plugin p, Chunk c) {
    if (Bukkit.isPrimaryThread()) return c.getEntities();
    if (FIRE_AND_FORGET) return new Entity[0];
    CompletableFuture<Entity[]> f = new CompletableFuture<>();
    Bukkit.getRegionScheduler().run(p, c.getWorld(), c.getX(), c.getZ(), t -> { try { f.complete(c.getEntities()); } catch (Exception e) { f.completeExceptionally(e); } });
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return new Entity[0]; }
  }
  public static boolean _cl(Plugin p, Chunk c, boolean g) {
    if (Bukkit.isPrimaryThread()) return c.load(g);
    if (FIRE_AND_FORGET) { Bukkit.getRegionScheduler().run(p, c.getWorld(), c.getX(), c.getZ(), t -> c.load(g)); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    Bukkit.getRegionScheduler().run(p, c.getWorld(), c.getX(), c.getZ(), t -> { try { f.complete(c.load(g)); } catch (Exception e) { f.completeExceptionally(e); } });
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
  public static boolean _cu(Plugin p, Chunk c, boolean s) {
    if (Bukkit.isPrimaryThread()) return c.unload(s);
    if (FIRE_AND_FORGET) { Bukkit.getRegionScheduler().run(p, c.getWorld(), c.getX(), c.getZ(), t -> c.unload(s)); return true; }
    CompletableFuture<Boolean> f = new CompletableFuture<>();
    Bukkit.getRegionScheduler().run(p, c.getWorld(), c.getX(), c.getZ(), t -> { try { f.complete(c.unload(s)); } catch (Exception e) { f.completeExceptionally(e); } });
    try { return f.get(API_TIMEOUT_MS, TimeUnit.MILLISECONDS); } catch (Exception e) { return false; }
  }
